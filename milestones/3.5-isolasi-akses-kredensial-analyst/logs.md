...
## 2026-08-17 -- Regresi pasca-milestone: GRANT ke tabel mart_cleaned yang diverifikasi Checkpoint 9 ternyata hilang lagi
Encountered: Saat investigasi laporan tim AI Chatbot soal `chatbot_authz_reader` (M4.4) gagal `permission denied`, ditemukan root cause sistemik di `scripts/reverse_etl/sync.py` (M2.4): RENAME-based swap yang dijalankan `reverse-etl-mart-cleaned.yml` terjadwal harian mengganti tabel `mart_cleaned.*` dengan objek fisik baru setiap sync, dan objek baru itu tidak pernah mewarisi GRANT SELECT apa pun. Dicek langsung ke `information_schema.role_table_grants`: 6 dari 7 tabel row-level milestone ini (`payroll`, `bookings`, `fnb_transactions`, `staff_shifts`, `maintenance_tickets`, `financial_summary`, `event_bookings`, `employee_performance`, `pricing_history`) semuanya 0 grant.
Klarifikasi penting terhadap catatan Checkpoint 9 di atas: catatan "keterbatasan visibility information_schema untuk non-superuser, `pg_class.relacl` menunjukkan grant persis cocok" itu **benar untuk kondisi 2026-08-09** (hari itu, `pg_class.relacl` memang menunjukkan grant yang benar-benar ada, bukan salah diagnosis) -- masalahnya muncul BELAKANGAN, dari sync harian terjadwal (diwire di M2.4, aktif sejak tanggal yang sama) yang berulang kali mengganti objek tabel dan mencabut grant itu tanpa pernah ada mekanisme yang menyadarinya. Jadi bukan salah baca `information_schema` waktu itu, tapi regresi nyata di kemudian hari akibat interaksi M2.4 (swap harian) x M3.5 (grant langsung ke tabel) yang tidak diantisipasi kedua milestone.
Fix (dieksekusi & didokumentasikan detail di `milestones/2.4-reverse-etl-postgresql/logs.md`): `scripts/reverse_etl/regrant_downstream_readers.py` -- re-GRANT SELECT ke seluruh 6 role domain M3.5 (`property_gm_analyst_reader` pulih otomatis lewat pewarisan role), dijalankan segera + di-wire ke `reverse-etl-mart-cleaned.yml` supaya tidak berulang.
Result: worked. Diverifikasi ulang lewat query nyata pakai kredensial masing-masing (`HR_ANALYST_READER_DB_URL`, `REVENUE_ANALYST_READER_DB_URL`, `FNB_ANALYST_READER_DB_URL`, `FACILITY_ANALYST_READER_DB_URL`, `SPA_EVENT_ANALYST_READER_DB_URL`, `CORPORATE_FINANCIAL_ANALYST_READER_DB_URL`, `PROPERTY_GM_ANALYST_READER_DB_URL`) -- 7/7 OK, bukan cuma diasumsikan dari GRANT berhasil dieksekusi.
