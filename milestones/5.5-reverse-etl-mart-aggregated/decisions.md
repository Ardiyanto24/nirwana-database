...
**Keputusan:** `scripts/reverse_etl_mart_aggregated/reindex_analyze.py` — `REINDEX TABLE`+`ANALYZE` dijalankan pasca setiap swap untuk SEMUA tabel yang baru disinkronkan (aman/no-op walau 0 index ada di tabel itu). Ditambah 1-2 index contoh di 1-2 tabel representatif (dipilih saat Fase 3 — kandidat: `fact_revenue_property_daily` pada `(property_id, period_date)`, sesuai kolom yang disebut arsitektur Bagian 9.3.2 untuk pola akses chatbot) khusus untuk membuktikan mekanisme `EXPLAIN ANALYZE` before/after bekerja nyata.
