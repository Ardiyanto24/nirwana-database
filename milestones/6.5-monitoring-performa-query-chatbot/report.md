...
- **`guests_contact_view` execution_time jauh lebih lambat** (2354-2494ms) dari 66 view lain — **diinvestigasi + partial fix** (follow-up 2026-08-11, lihat `logs.md`). Root cause: `bookings`/`spa_bookings`/`guests` tidak punya index sama sekali (domain "guests" tidak masuk 6 checkpoint M3.3). `idx_guests_guest_id` ditambahkan — perbaikan nyata & permanen (menghilangkan sort-disk di sisi `guests`). Sisa ~85-90% biaya (`Seq Scan`+external sort disk pada gabungan `bookings`+`spa_bookings` ~318K baris untuk `guests_contact_view` MAUPUN `guests_profile_view` — subquery `lap` keduanya identik, temuan baru) **TIDAK bisa diperbaiki lewat index apapun** — dikonfirmasi empiris (`enable_seqscan=off` tetap gagal membangun Merge Append untuk bentuk `UNION ALL`+`DISTINCT ON` ini, PostgreSQL 17.6). Fix sungguhan butuh tabel ringkasan pre-computed (mengubah JOIN target view) — di luar scope "index" yang disepakati, didokumentasikan sebagai limitation per keputusan eksplisit user.
...
- **Kalau `guests_contact_view`/`guests_profile_view` suatu saat perlu dituntaskan sepenuhnya (bukan cuma `idx_guests_guest_id`):** perlu tabel ringkasan pre-computed 1 baris/guest (mis. `mart_cleaned.guest_last_active_property`, di-refresh tiap reverse-ETL sync sama seperti index M3.3/`idx_guests_guest_id`), lalu kedua view LEFT JOIN ke situ alih-alih subquery `lap` live. Ini mengubah JOIN target view (bukan cuma tambah index) — kolom/access-control TIDAK berubah, tapi tetap lebih besar dari scope "pure performance fix" yang disepakati saat follow-up 2026-08-11 (lihat `logs.md`), jadi butuh keputusan/milestone terpisah kalau diprioritaskan.
