...
Baca histori occupancy dari `mart_cleaned` (BigQuery) via client, hitung forecast naif (moving average occupancy 14-30 hari terakhir per property x room_type + noise kecil untuk `confidence_score`), tulis ke `ml_output.predictions` lewat `CREATE OR REPLACE TABLE x AS SELECT * FROM x UNION ALL SELECT <hasil scoring baru>` — DDL murni, kompatibel Sandbox mode (sama pola `fact_revenue_pace_booking_snapshot` M5.3). `model_name='occupancy_forecast'`, `model_version` string statis (mock, tidak perlu versioning sungguhan). Dataset `ml_output` dibuat idempotent (`create_dataset(..., exists_ok=True)`) di script yang sama.
