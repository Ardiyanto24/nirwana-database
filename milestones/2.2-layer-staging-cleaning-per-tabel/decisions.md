...
- [ ] Task 3: Definisikan mapping `employees.department` 19→8 eksplisit — Acceptance: 19 nilai asli termapping ke 8 nilai baku, didokumentasikan — Verify: review manual — S
- [ ] Task 4: Model dbt `stg_corporate_master__employees` (trim `full_name`, mapping `department`, parse `hire_date`, preserve null `role_title`) — Acceptance: row count = raw, distinct `department`=8, `hire_date` bertipe DATE — Verify: query pembanding — M
- [ ] Task 5: Model dbt `stg_corporate_master__guests` (normalisasi `phone`, `nationality` case/trim, preserve typo/null/duplikat) — Acceptance: row count = raw, distinct format `phone`=1 — Verify: query pembanding — M
