...
- [ ] **Checkpoint 2** — `scripts/monitoring_warehouse/titik_config.py` (9 baris titik 1-9) — Acceptance: workflow filename + step-name-substring cocok persis isi YAML — Verify: grep silang — S
- [ ] **Checkpoint 3** — `scripts/monitoring_warehouse/snapshot_pipeline_run.py` — Acceptance: insert benar dari run GitHub Actions nyata yang sudah ada — Verify: cocokkan hasil query vs `gh run view` — M
- [ ] **Checkpoint 4** — `.github/workflows/monitoring-warehouse-pipeline-log.yml` + verifikasi KK1 — Acceptance: listener otomatis terpicu, `pipeline_run_status` benar — Verify: trigger nyata + query — S
- [ ] **Checkpoint 5** — Verifikasi KK2 (riwayat terakumulasi) + cakupan titik 2/3 & 6/7 (granularity benar) — Verify: trigger berulang + query — XS
- [ ] **Checkpoint 6** — Update peta M6.1 + `logs.md` + `report.md` — Verify: cocokkan KK1/KK2 dengan bukti Checkpoint 4/5 — S
