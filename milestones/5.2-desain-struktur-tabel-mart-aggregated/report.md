...
- `docs/07-mart-aggregated/DataSchema-mart-aggregated.md` — 27 dimension table, 45 fact table (Revenue 5, F&B 8, Facility/Ops 9, Spa & Event 6, HR 6, Corporate/Financial 9, Kasus Khusus 2), bagian Audit PII.
...
- **Update Milestone 5.7 (2026-08-09): `dim_employee` terlewat menyertakan `property_id`** — meski KK#2 milestone ini sendiri mewajibkan `property_id` sebagai kolom filter wajib di seluruh skema, `dim_employee` adalah satu-satunya dimension yang terlewat. Tidak terdeteksi saat M5.2/M5.3 (tidak ada di Known Gaps aslinya) — baru ditemukan oleh Data Analyst Serving (M3.2) dan diselesaikan lewat mekanisme pengajuan M5.6, lihat `docs/07-mart-aggregated/pengajuan-perubahan-cakupan.md`.
