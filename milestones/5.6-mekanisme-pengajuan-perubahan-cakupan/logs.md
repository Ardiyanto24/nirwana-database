...
**Verifikasi tambahan lewat GitHub Actions dibatalkan oleh user** -- `transform-mart-aggregated.yml` di-trigger manual (`workflow_dispatch` langsung, bukan lewat `transform-mart-cleaned.yml`) untuk membuktikan alur otomatis penuh, tapi run jadi lambat karena sensor `ml_output` (lookback 90 menit) tidak menemukan data fresh (scoring terakhir sudah lewat window itu, dan trigger manual ini tidak ikut memicu ulang `scoring-occupancy-forecast.yml`) -- sensor akan menghabiskan hampir 60 menit menunggu timeout sebelum lanjut. User meminta dihentikan ("hentikan saja, karena toh itu juga ML nya masih berupa buatan belum aslinya") -- run dibatalkan (`gh run cancel`). **Tidak mengurangi validitas KK2**: perubahan sudah dibuktikan sampai ke BigQuery dan Postgres sungguhan lewat jalur manual (`promote.py`/`sync.py` di atas) -- yang batal cuma lapisan pembuktian tambahan "otomatis lewat CI," bukan tindak lanjut inti KK2 itu sendiri. Dicatat sebagai keputusan sadar, bukan gap yang disembunyikan.

## 2026-08-08 -- Checkpoint 6: tutup siklus + update dokumentasi (Fase 5)

Entri backlog "Tindak lanjut" diperbaiki (menghapus klaim verifikasi CI yang dibatalkan, diganti deskripsi akurat: verifikasi manual langsung terhadap BigQuery/Postgres). `DataSchema-mart-aggregated.md` dan `Metadata-mart-aggregated.md` diupdate -- kolom `in_watchlist` didokumentasikan, catatan lama "belum ada kolom flag" diganti "Koreksi (M5.6)" menjelaskan threshold final + kalibrasi.

Cross-reference Known Gaps ditambahkan (bukan menulis ulang isi lama) di 3 report.md: `milestones/5.1-.../report.md`, `5.2-.../report.md`, `5.3-.../report.md` -- masing-masing dapat 1 baris "Update Milestone 5.6 (2026-08-08): threshold watchlist HR diselesaikan", dengan catatan threshold SLA breach Facility/Ops masih terbuka (belum diajukan, di luar scope trial ini).
