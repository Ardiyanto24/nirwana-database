"""
Milestone 2.4 Task 6 -- controlled test proving RENAME-based swap doesn't
cause query failures against the live table (Kriteria Keberhasilan #2).

Runs a tight polling loop (SELECT COUNT(*) against mart_cleaned.properties)
...
Usage: python scripts/reverse_etl/test_no_downtime_swap.py
"""
import threading
import time
        ...
        while not stop_event.is_set():
            try:
                with conn.cursor() as cur:
                    cur.execute("SELECT COUNT(*) FROM mart_cleaned.properties")
                    cur.fetchone()
                results["queries"] += 1
...
def main():
    ...
    try:
        for i in range(SYNC_CYCLES):
            sync_table(bq_client, pg_conn, "properties")
