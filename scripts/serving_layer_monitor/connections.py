"""
Connection helpers for Milestone 6.6 serving-layer monitoring.

Copy of scripts/chatbot_perf_monitor/connections.py (not imported --
project convention since M2.1). Dual-instance need: SERVING project
(pg_stat_user_tables/pg_total_relation_size, mart_cleaned+mart_aggregated --
via SERVING_DB_URL for admin setup) and production Supabase (monitoring.* --
via SUPABASE_DB_URL) for the new snapshot/alert tables (monitoring stays
centralized regardless of where the observed system physically lives).
"""
import os
import re
...
def write_env_var(key, value):
    """Read/filter-out-old-key/append/rewrite .env -- same idiom as M3.5/M3.6/M4.3/M4.5/M6.5.
