...
- **Keputusan:** **DISETUJUI**. Threshold: `in_watchlist = coalesce(absence_deviation_ratio > 1.5, false) OR coalesce(late_deviation_ratio > 1.5, false)` — rasio deviasi lebih dari 1.5x baseline individu (absence ATAU late), konsisten filosofi "within-entity-over-time" yang sudah dikunci sejak M5.1/M5.2 (deviasi dari baseline individu sendiri, bukan rate absolut lintas-karyawan). `coalesce(..., false)`: bulan pertama karyawan (baseline belum ada, ratio `NULL`) dianggap belum bisa dinilai, bukan otomatis ter-flag.
