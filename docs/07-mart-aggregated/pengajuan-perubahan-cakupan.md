...
- **Keputusan:** **DISETUJUI, jalur cepat** — sesuai aturan keputusan mekanisme M5.6 sendiri ("dampak rendah + data tersedia penuh → jalur cepat DISETUJUI"). Temuan pengaju diverifikasi ulang langsung dari kode sebelum keputusan diambil (`stg_corporate_master__employees.sql`, `mart_cleaned__employees.sql`, `dim_employee.sql`, `fact_hr_employee_monthly.sql`, `fact_hr_employee_performance_semester.sql` dibaca langsung) — 100% akurat, bukan kesalahpahaman. Ditindaklanjuti lewat Milestone 5.7 (`milestones/5.7-perubahan-cakupan-dim-employee-property-id/`), bukan hotfix ad-hoc, sesuai aturan proses mekanisme ini sendiri.
- **Tindak lanjut:** `warehouse/models/mart_aggregated/corporate_master/dim_employee.sql` diupdate (`property_id` ditambahkan) + test `relationships` ke `dim_property`, dipromosikan ke `mart_aggregated` (BigQuery, `promote.py --select dim_employee`) dan disinkronkan ke serving PostgreSQL (`sync.py --table dim_employee`) — diverifikasi langsung terhadap kedua sisi infrastruktur sungguhan (755/755 baris, 100% non-null, distribusi per-properti P01-P06 identik BigQuery vs Postgres). **Temuan tambahan saat verifikasi**: reverse-ETL swap M5.5 sempat crash karena `analyst_views` (M3.2) yang baru dibuat mengikat ke tabel lama lewat OID — diperbaiki di `sync.py` (kegagalan drop tabel lama sekarang jadi warning, bukan crash) dan dibuktikan lewat 2 siklus nyata; otomasi penuh reapply-view dicatat sebagai keputusan tertunda terpisah (`docs/keputusan-tertunda.md`). Detail lengkap di `milestones/5.7-.../logs.md`. **Catatan cakupan**: perbaikan ini murni di layer `mart_aggregated` (kolom `property_id` kini tersedia untuk di-join) — 3 view `analyst_views` terkait (`v_hr_employee_monthly`, `v_hr_employee_performance_semester`, `v_hr_watchlist_monthly`) belum diupdate untuk men-SELECT `property_id` (di luar scope kepemilikan `mart_aggregated`), tetap tercatat sebagai Known Gap M3.2 yang kini sudah unblocked kalau pemilik M3.2/M3.4 ingin melanjutkannya.
- **Status:** Selesai (2026-08-09) — lihat `milestones/5.7-perubahan-cakupan-dim-employee-property-id/report.md`.
